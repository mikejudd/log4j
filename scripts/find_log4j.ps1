...
foreach ($drive in $drives) {
    $robocopyexitcode = (Start-Process robocopy  -ArgumentList "$drive c:\DOESNOTEXIST *.jar /S /XJ /L /FP /NS /NC /NDL /NJH /NJS /r:0 /w:0 /LOG:$env:temp\log4jfilescan.csv" -Wait).exitcode
    if ($? -eq $True) {
